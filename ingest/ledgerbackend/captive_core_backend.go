...
// Captive diamnet-core backend needs to initialize Diamnet-Core state to be
...
func (c *CaptiveDiamnetCore) GetLedger(ctx context.Context, sequence uint32) (xdr.LedgerCloseMeta, error) {
	...
	if c.isClosed() {
		return xdr.LedgerCloseMeta{}, errors.New("diamnet-core is no longer usable")
	}
	...
	if c.diamnetCoreRunner == nil {
		return xdr.LedgerCloseMeta{}, errors.New("diamnet-core cannot be nil, call PrepareRange first")
	}
	if c.closed {
		return xdr.LedgerCloseMeta{}, errors.New("diamnet-core has an error, call PrepareRange first")
	}
...
func (c *CaptiveDiamnetCore) GetLatestLedgerSequence(ctx context.Context) (uint32, error) {
	...
	if c.isClosed() {
		return 0, errors.New("diamnet-core is no longer usable")
	}
	if c.prepared == nil {
		return 0, errors.New("diamnet-core must be prepared, call PrepareRange first")
	}
	if c.diamnetCoreRunner == nil {
		return 0, errors.New("diamnet-core cannot be nil, call PrepareRange first")
	}
	if c.closed {
		return 0, errors.New("diamnet-core is closed, call PrepareRange first")
...
// temporary files. Note, once a CaptiveDiamnetCore instance is closed it can no longer be used and
